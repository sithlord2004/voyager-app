...
import { daysUntil, createTrip } from '../lib/db.js'
...
function TripRow({ trip, docCount }) {
...
export default function Trips({ trips, documents, reload }) {
  ...
  return (
    <div>
      ...
      {trips.map(t => (
        <TripRow key={t.id} trip={t} docCount={documents.filter(d => d.tripId === t.id).length || t.travellerIds.length} />
      ))}
