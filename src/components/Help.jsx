  ...
  return (
    <div>
      ...
      <div className="card" style={card}>
        ...
        <P><b>Quick way to set up sync on another phone:</b> on the device that already has sync working, go to <b>Settings → Cloud sync → 📲 Invite a device</b>. Show the <b>QR code</b> to the other phone's camera (or send the link). It fills in all the sync settings automatically — one tap, no typing. The other phone just taps <b>Join family</b>. (The link carries your sync secret, so only share it with people you want on the same family.)</P>
        <P><b>Setting up a separate group</b> (e.g. grandparents who want to share with each other, not with you): in that same invite window switch to the <b>✨ New group</b> tab, give it a name, and send the one code/link to everyone in that group. They'll sync trips and packing with each other, completely walled off from your family. (Shared <i>documents</i> also need that group to share a passphrase via a restored backup, but trips work right away.)</P>
